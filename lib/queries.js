...
module.exports = ({ model, siapi }) => {
  ...
  const defaultPopulate = (options = {}) =>
    model.associations
      ...
      .map(ast => {
        const assocModel = siapi.db.getModelByAssoc(ast);
    ...
    for (let key of componentKeys) {
        ...
        if (repeatable === true) {
          const components = await Promise.all(
            componentValue.map(value => {
              return siapi.query(component).create(value, { session });
            })
          );
          ...
          const componentEntry = await siapi.query(component).create(componentValue, { session });
          entry[key] = [
            {
              kind: componentModel.globalId,
              ref: componentEntry.id,
      ...
      if (type === 'dynamiczone') {
        ...
        const componentsArr = dynamiczones.map(({ __component, entity }) => {
          const componentModel = siapi.components[__component];
  ...
  async function updateComponents(entry, values, { session = null } = {}) {
    ...
    const updateOrCreateComponent = async ({ componentUID, value }) => {
      ...
      const query = siapi.query(componentUID);
    ...
    for (let key of componentKeys) {
      ...
      if (type === 'component') {
        ...
        const componentModel = siapi.components[componentUID];
        const componentValue = values[key];
    ...
    for (let key of componentKeys) {
      ...
      if (type === 'dynamiczone') {
        if (Array.isArray(entry[key]) && entry[key].length > 0) {
          ...
          await Promise.all(
            Object.keys(deleteMap).map(componentUID => {
              return siapi.query(componentUID).delete(
                {
