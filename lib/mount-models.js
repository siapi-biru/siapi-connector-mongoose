...
module.exports = async ({ models, target }, ctx) => {
        ...
        morphAssociations.forEach(association => {
          if (
            ...
            switch (association.nature) {
              case 'oneMorphToOne':
                returned[association.alias] = refToSiapiRef(returned[association.alias][0]);
      ...
      Model.on('index', error => {
        if (error) {
          if (error.code === 11000) {
            ...
            siapi.log.error(`An index error happened, it wasn't applied.\n\t- ${error.message}`);
          }
        }
      });
    };
  ...
  for (const model of Object.keys(models)) {
    ...
    // run migrations
    await siapi.db.migrations.run(migrateSchema, {
      definition,
      previousDefinition,
      model: modelInstance,
      ORM: instance,
...
const buildRelation = ({ definition, model, instance, attribute, name }) => {
  ...
  const getRef = (name, plugin) => {
    return siapi.db.getModel(name, plugin).globalId;
  };
