...
module.exports = function(siapi) {
  ...
  function initialize() {
    ...
    const connectionsPromises = mongooseConnections.map(async connectionName => {
      ...
      _.defaults(connection.settings, siapi.config.hook.settings.mongoose);
      ...
      const { authenticationDatabase, ssl, debug } = _.defaults(
        connection.options,
        uriOptions,
        siapi.config.hook.settings.mongoose
      );
      ...
      _.set(siapi, `connections.${connectionName}`, instance);
  ...
  function mountApis(connectionName, ctx) {
    const options = {
      models: _.pickBy(siapi.models, ({ connection }) => connection === connectionName),
      target: siapi.models,
    };
  ...
  async function destroy() {
    await Promise.all(
      mongooseConnections.map(connName => {
        const mongooseConnection = siapi.connections[connName];
