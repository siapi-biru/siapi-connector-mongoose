      ...
      switch (association.nature) {
        ...
        case 'manyMorphToOne': {
          newValue.forEach(obj => {
            const refModel = siapi.db.getModel(obj.ref, obj.source);
        ...
        case 'manyToManyMorph': {
          ...
          const model = siapi.db.getModel(details.model || details.collection, details.plugin);
            ...
            if (Array.isArray(entry[association.alias])) {
              return Promise.all(
                entry[association.alias].map(val => {
                  const targetModel = siapi.db.getModelByGlobalId(val.kind);
